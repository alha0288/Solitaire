...
def loadImage(name):
    image = pygame.image.load(os.path.join(Settings.imagePath, name + Settings.imageType))
...
class AbstractTilePile(AbstractPile):
    ...
    def addSingle(self, card):
        if self.isEmpty():
            ...
            if last_card.faceUp:
                card.setPosition((last_card.rect.x, last_card.rect.y + self.add_space))
            # If the last card is faceDown, it means the card should be added with the init_space
            else:
                card.setPosition((last_card.rect.x, last_card.rect.y + self.init_space))
